...
describe('Компонента Chat', () => {
  ...
  describe('render', () => {
    ...
    it('Содержит input с классом input-message', () => {
      expect(wrapper.find('input.input-message')).toHaveLength(1);
    });
  });
  ...
  describe('Методы компоненты', () => {
    ...
    it('Содержит метод changeInputMessage', () => {
      expect(wrapper.instance().changeInputMessage).toBeDefined();
    });

    it('Содержит метод sendMessageOnEnter', () => {
      expect(wrapper.instance().sendMessageOnEnter).toBeDefined();
    });
  });
