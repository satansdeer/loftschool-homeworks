...
class RoverViewer extends PureComponent {
  ...
  changeSol = value => {
    ...
    changeSol(value);
    if (!photos['curiosity'][value])
      fetchPhotosRequest({ name: 'curiosity', sol: value, key: apiKey });
    if (!photos['opportunity'][value])
      fetchPhotosRequest({ name: 'opportunity', sol: value, key: apiKey });
    if (!photos['spirit'][value])
      fetchPhotosRequest({ name: 'spirit', sol: value, key: apiKey });
  };
